...
1-3 때문에 이 방법밖에 없다. 부작용은 **원천이 페이지 중간에 요청보다 적게 주면 조기 종료**한다는 것인데, 이 경우에도 결정 3의 직접 증거(`seen < totalCount`)가 절단을 잡아낸다. 즉 조기 종료가 조용한 데이터 손실로 이어지지 않는다. 두 판정이 서로를 보완하는 구조다.
...
### 놓치기 쉬운 것: `seen`과 `collected`는 다른 수다
...
| 지켜야 할 것 | 깨지면 |
|---|---|
| `seen`은 필터 **이전** 행 수 | 불량 행이 전부 절단으로 오인 → `sync_run.truncated` 거짓 양성 |
| 절단 판정은 **직접 증거 우선** | 상한 == 원천 크기일 때 오판 (500/500) |
