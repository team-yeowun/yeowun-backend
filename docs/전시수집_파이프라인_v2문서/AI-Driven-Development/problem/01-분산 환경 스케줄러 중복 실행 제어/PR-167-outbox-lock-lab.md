...
- 목표
    ...
    - 원시 파일(요약·계기판 원문·로그·10초 표본)은 저장소 규칙(`.gitignore`의 `_workspace/`)에 따라 로컬에만 보존하고, 저장소에는 해석 문서와 재현 도구만 올립니다. 원시가 필요하면 요청해 주세요.
- 결과
    - `lab/outbox-lock/` 아래에 실험 무대와 실행 스크립트, 변형 9개, 관측 스크립트가 들어왔어요.
    - 변형 9개를 각 1회 실행한 결과의 해석 문서(`action/` 5편 · `evidence/`)와 재현 도구가 들어옵니다. 문서의 숫자는 로컬 원시(`_workspace/03_measurer_raw/`)에서 역추적됩니다.
