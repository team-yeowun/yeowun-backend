 ...
 *   <li>최근접 순위였다면 N=10 에서 p95 가 최댓값과 같아져 백분위로 읽히지 않는다(게이트 F-07 의 취지)</li>
 *   <li>max 를 따로 실어 보내 p95 와 최댓값이 붙었는지 독자가 직접 본다</li>
 * </ul>
 */
record OutboxLabStats(int n, double p50, double p95, double min, double max, double mean, double stdev) {

	static final String P95_METHOD = "linear interpolation (R-7, numpy 기본)";

	static OutboxLabStats of(double[] rawValues) {
		if (rawValues.length == 0) {
			return new OutboxLabStats(0, 0, 0, 0, 0, 0, 0);
		}
		...
		return new OutboxLabStats(values.length, percentile(values, 0.50), percentile(values, 0.95),
