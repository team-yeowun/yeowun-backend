 ...
 *
 * <p>매일 01시(설계 D5) {@code syncCatalog} <b>하나만</b> 부른다 — 이 클래스는 트리거일 뿐이라 "이번 회차에 무엇을 하는가"를
 * 알지 못한다. 목록 수집·스테이징도, 영업시간 확인 대상 스윕도 그 메서드 안이고, 실제 외부 조회(상세·장르·승격·
 * 영업시간)는 거기서 발행된 이벤트를 릴레이가 소비한다.
 *
...
public class ExhibitionSyncScheduler {
	...
	/** 매일 01시: 목록 수집·스테이징(이벤트 발행) — 실행은 릴레이 소비. 실패해도 다음 주기에 재시도. */
	@Scheduled(cron = "${app.exhibition.sync.cron:0 0 1 * * *}")
	public void syncDaily() {
		...
		try {
			...
			// 전부 syncCatalog 안이고, 실제 조회(상세·장르·승격·영업시간)는 이벤트를 릴레이가 소비한다.
