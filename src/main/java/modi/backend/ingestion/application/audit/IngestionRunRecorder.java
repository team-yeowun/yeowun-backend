...
/**
 * 수집 런 감사(append-only) 기록 협력 객체 — 동기화 루프({@code ExhibitionIngestionOrchestrator})가 실행 종료 시
 * run 하나만 넘겨 위임한다(집계는 run이 이미 누적해 갖고 있다). 동기화 요약 로그도 여기 소유다.
 *
 * <p>best-effort 삼킴이다: 부가 기록이라 실패해도 동기화 결과를 깨지 않는다. 콜 감사
 * ({@link ExternalApiCallLogRecorder})와 달리 {@code REQUIRES_NEW}가 아니다 — 이 기록은 루프 종료 후
 * 트랜잭션 밖에서 한 번 불리므로 분리할 트랜잭션 자체가 없다(현행 유지).
 */
@Component
...
public class IngestionRunRecorder {
	...
	public void record(IngestionRun run) {
		if (run.hasActivity()) {
			log.info("전시 동기화: 수집 {} / 신규스테이징 {} / 갱신 {} / 기간스킵 {} / 실패연기 {}",
					run.getCollected(), run.getInserted(), run.getCompleted(), run.getSkipped(), run.getDeferred());
		}
		try {
