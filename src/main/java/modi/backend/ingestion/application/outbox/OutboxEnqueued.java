...
/**
 * 아웃박스에 메시지가 적재됐음을 알리는 스프링 이벤트 — <b>디스패치 적재 알림</b>다(ADR-10 "이벤트=적재 알림, 테이블=엔진").
 *
 * <p>enqueue 트랜잭션이 커밋된 뒤({@code AFTER_COMMIT}) 릴레이가 이를 받아 소비를 앞당긴다(폴링 주기 대기 제거).
 ...
 *
 * @param messageType 적재된 메시지 종류(관측·로그용 — 릴레이는 종류와 무관하게 도래분 전체를 소비한다)
 */
