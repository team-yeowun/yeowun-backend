...
public class GooglePlaceClient implements PlaceHoursProvider {
    ...
    public Optional<PlaceHoursResult> fetch(String placeName, String placeAddr) {
        GooglePlaceDto.SearchTextRequest request = new GooglePlaceDto.SearchTextRequest(
        ...
        GooglePlaceDto.GooglePlaceResponse response = googleMapsRestClient.post()
        ...
        // 응답 자체가 없는 경우와 응답은 왔으나 후보가 없는 경우를 GooglePlaceResponse.empty()로 합류시켜
        // 파이프라인 진입점(toPlaceHours) 하나로 처리한다(결측 분기 제거).
        return Optional.ofNullable(response)
                .orElseGet(GooglePlaceResponse::empty)
                .toPlaceHours();
    }
