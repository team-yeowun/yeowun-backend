...
public class ExternalApiCallLog {
	...
	/**
	 * AI 호출 1건 — 요청 모델을 함께 남긴다(모델별 호출량·429 비율 집계용). 장르는 draft당 개별 분류라
	 * 대상({@code external_id})이 항상 하나다 — requestKey로 남긴다.
	 */
	public static ExternalApiCallLog ai(ExternalApi api, String model, String requestKey, ExternalApiOutcome outcome,
			LocalDateTime calledAt) {
		return new ExternalApiCallLog(api, model, requestKey, outcome, calledAt);
	}
}
