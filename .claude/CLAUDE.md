...
- **전시 예외(애그리거트 루트)**: 전시·전시장 부속(상세·장르·작가조인·영업시간)은 개별 리포 ❌ — 쓰기는 `ExhibitionRepository`/`ExhibitionPlaceRepository`의 `applyXxx`(루트 행 미변경 부분 갱신) 경유, 서빙 목록/탐색 조회는 `ExhibitionQueryRepository`. 부속 JpaRepo는 Impl 내부 구현 세부라 application 주입 ❌. 기록성 테이블(벤더 Snapshot·History)은 포트 없이 JpaRepository 직사용(단 `ExternalApiCallLogRepository`는 감사 포트라 존치 — REQUIRES_NEW+예외 삼킴은 `application/audit/ExternalApiCallLogRecorder` 소유).
...
- **ingestion→코어 접근은 `application/exhibition/contract/` 인터페이스만**(ExhibitionRegistrar=승격 등록·PlaceHoursGateway=영업시간 정준 통로(구 PlaceHoursBackfill — 개명, 현역). ExhibitionBackfill=레거시 뒤채움은 레거시 데이터 부재 확인 후 삭제 — 재도입 ❌, 뒤채움 필요 시 일회성 배치로) — 코어 리포 직주입 ❌. 계약 어휘는 코어 소유(원시값·코어 enum — ingestion record가 경계 넘기 ❌). 코어→ingestion 참조 ❌(javadoc @link 포함).
- **수집 = 이벤트 아웃박스**(`docs/ingestion-redesign.md`가 진실): 아웃박스 레코드는 발행자의 사실(이벤트 — DRAFT_STAGED·DETAIL_FETCHED·DRAFT_READY·PLACE_HOURS_STALE)이지 커맨드 ❌. 발행은 상태 소유 Service의 반영 tx(원자), **이벤트→다음 스텝 매핑은 `ExhibitionIngestionOrchestrator` 한 파일만**(오케스트레이터엔 try/catch ❌ — 예외 번역은 outbox drain, 축별 실패 처리는 그 서비스 소유). 진입점도 `syncCatalog` 하나(발견=목록 스테이징+영업시간 스윕, 실행은 전부 릴레이 드레인). 인메모리 사가(한 메서드 완주) ❌ — 스텝별 이벤트로 전진, 스텝 = [판정→외부 호출(tx 밖, 직후 콜 감사)→반영(tx+이벤트)] 3박자. 승격 = DRAFT_READY 멱등 소비(Registrar). 장르는 **draft당 개별 분류**(classifyAll 배치 재도입 ❌). 장르 분류 포트(GenreClassifier)·영업시간 어휘는 코어 소유(CUSTOM 등록도 사용), 구현은 ingestion infra. 보상(compensation) 설계 ❌ — 전진 복구(재시도+영구 실패 시 draft FAILED 가시화)만.
...
- **infra 외부 어댑터 경로 규칙 = `<슬라이스/도메인 스코프>/<기관>`**(예: ingestion/infra/{culture,google,ai,mock,failover} + 소유권 패키지 {snapshot,audit,draft,outbox}, infra/ai/{claude,gemini}=record·remind 공유) — infra/genre·infra/place 같은 스코프 없는 기능 패키지 재생성 ❌. ingestion application은 이벤트 발행자 축(culture·genre·place·draft·outbox·audit + 루트 `ExhibitionIngestionOrchestrator` 유일).
